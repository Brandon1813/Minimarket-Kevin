<?php
	...
	$check_usuario=$check_usuario->query("SELECT * FROM usuarioInv WHERE usuario_id='$id'");
    ...
    $check_admin=$check_admin->query("SELECT usuario_usuario,usuario_clave FROM usuarioInv WHERE usuario_usuario='$admin_usuario' AND usuario_id='".$_SESSION['id']."'");
    ...
    if($email!="" && $email!=$datos['usuario_email']){
        if(filter_var($email, FILTER_VALIDATE_EMAIL)){
            ...
            $check_email=$check_email->query("SELECT usuario_email FROM usuarioInv WHERE usuario_email='$email'");
    ...
    if($usuario!=$datos['usuario_usuario']){
	    $check_usuario=conexion();
	    $check_usuario=$check_usuario->query("SELECT usuario_usuario FROM usuarioInv WHERE usuario_usuario='$usuario'");
	    if($check_usuario->rowCount()>0){
	        echo '
	            <div class="notification is-danger is-light">
	                <strong>¡Ocurrio un error inesperado!</strong><br>
    ...
    $actualizar_usuario=$actualizar_usuario->prepare("UPDATE usuarioInv SET usuario_nombre=:nombre,usuario_apellido=:apellido,usuario_usuario=:usuario,usuario_clave=:clave,usuario_email=:email WHERE usuario_id=:id");
