...
require_once('./models/config.php'); //Llamando a la conexión para BD
...
$pdf->Cell(35,6,'Venta Total',1,0,'C',1);
...
$minimarket_kevin = ("SELECT * FROM ventas WHERE (Ven_Fecha>='$fechaInit' and Ven_Fecha<='$fechaFin') ORDER BY Ven_Fecha ASC");

$query = mysqli_query($con, $minimarket_kevin);

while ($dataRow = mysqli_fetch_array($query)) {
        $pdf->Cell(40,6,($dataRow['ID']),1,0,'C');
        $pdf->Cell(60,6,$dataRow['Ven_Nombre'],1,0,'C');
        $pdf->Cell(35,6,('$ '. $dataRow['Ven_Total']),1,0,'C');
        $pdf->Cell(35,6,(date('m-d-Y', strtotime($dataRow['Ven_Fecha']))),1,1,'C');
    }


//$pdf->AddPage(); //Agregar nueva Pagina

$pdf->Output('Reporte'.date('d_m_y').'.pdf', 'I'); 
